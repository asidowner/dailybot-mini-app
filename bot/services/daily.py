...
def get_daily_data(date_from: date) -> list[DailyModel]:
    ...
    query = select(DailyModel).where(DailyModel.created_at >= date_from)
...
def save_daily_data(user: UserModel, yesterday_tasks: str, today_plan: str, issues: str | None = None) -> None:
    ...
    query = insert(DailyModel).values(user=user, yesterday_tasks=yesterday_tasks, today_plan=today_plan, issues=issues)
